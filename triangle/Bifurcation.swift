...
struct BifurcationView: View {
  ...
  init() {
    ...
    let updateAttractorBase = library.makeFunction(name: "updateBifurcationBase")!
    self.attractorPipeline = try! device.makeComputePipelineState(function: updateAttractorBase)

    let updateBifurcationVertexes = library.makeFunction(name: "updateBifurcationVertexes")!
    self.vertexPipeline = try! device.makeComputePipelineState(function: updateBifurcationVertexes)
  }

...
struct MorphingSphereMetalView: View {
  ...
  func triangleMesh() throws -> LowLevelMesh {
    ...
    mesh.withUnsafeMutableBytes(bufferIndex: 0) { rawBytes in
      ...
      vertices[0] = VertexData(
        position: [-1, -1, 0], normal: SIMD3<Float>(0, 1, 1), uv: SIMD2<Float>(1.0, Float(1)))
      vertices[1] = VertexData(
        position: [1, -1, 0], normal: SIMD3<Float>(0, 1, 1), uv: SIMD2<Float>(1, Float(1)))
      vertices[2] = VertexData(
        position: [0, 1, 0], normal: SIMD3<Float>(0, 1, 1), uv: SIMD2<Float>(1, Float(1)))
    }

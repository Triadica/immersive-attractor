...
struct MobiusSpheresView: View {
  ...
  init() {
    ...
    let updateVertexes = library.makeFunction(name: "updateMobiusSpheresVertexes")!
  ...
  var body: some View {
    GeometryReader3D { proxy in
      RealityView { content in
        ...
        // Gesture support disabled - using GameManager controller input instead
        // rootEntity.components.set(GestureComponent())
        // rootEntity.components.set(InputTargetComponent())
        // Collision disabled since we're using controller
      ...
      // MARK: - Gesture support disabled - using GameManager controller input instead
      // .gesture(
      //   DragGesture()
  ...
  var sphereVertexes: [SIMD3<Float>] {
    makeSphereWithIterate(times: 4)
  }

  let sphereCount: Int = 57
  ...
  func createPingPongBuffer() -> PingPongBuffer {
    ...
    var index: Int = 0
    func writeSphere(center: SIMD3<Float>, radius: Float) {
      for j in 0..<shape.count {
        let idx = index * shape.count + j
    ...
    writeSphere(center: offset + SIMD3<Float>(0, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0.5, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(-0.5, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0, 0.5, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0, -0.5, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0, 0, 0.5), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0, 0, -0.5), radius: 0.25)

    offset = SIMD3<Float>(1.5, 0, 0)
    writeSphere(center: offset + SIMD3<Float>(0, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0.5, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(-0.5, 0, 0), radius: 0.25)
    writeSphere(center: offset + SIMD3<Float>(0, 0.5, 0), radius: 0.25)
    ...
    writeSphere(center: SIMD3<Float>(0, 0, 0), radius: 0.75)
    writeSphere(center: SIMD3<Float>(1.5, 0, 0), radius: 0.75)
    writeSphere(center: SIMD3<Float>(-1.5, 0, 0), radius: 0.75)
    writeSphere(center: SIMD3<Float>(0, 1.5, 0), radius: 0.75)
    writeSphere(center: SIMD3<Float>(0, -1.5, 0), radius: 0.75)
    writeSphere(center: SIMD3<Float>(0, 0, 1.5), radius: 0.75)
    writeSphere(center: SIMD3<Float>(0, 0, -1.5), radius: 0.75)

    writeSphere(center: SIMD3<Float>(0, 0, 0), radius: 0.75 * 3)
